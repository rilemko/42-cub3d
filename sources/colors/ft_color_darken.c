...
/*   Updated: 2023/07/24 22:42:49 by mconreau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_color_darken(t_rgb *rgb, double percentage)
{
	...
	rgb->channel_r = ft_ternary_int(final < 0, 0, final);
	final = rgb->channel_g - (percentage / 100 * rgb->channel_g);
	rgb->channel_g = ft_ternary_int(final < 0, 0, final);
	final = rgb->channel_b - (percentage / 100 * rgb->channel_b);
	rgb->channel_b = ft_ternary_int(final < 0, 0, final);
}
